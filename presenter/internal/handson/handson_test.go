...
// TestHandle_Success は presenter ロールでの正常なハンズオン指示配信を検証する。
func TestHandle_Success(t *testing.T) {
	...
	err := h.Handle(context.Background(), "default", "conn1", "Write hello world", "your code here")
	if err != nil {
	...
	expected := `{"type":"hands_on","instruction":"Write hello world","placeholder":"your code here"}`
	if string(capturedPayload) != expected {
		t.Errorf("expected %s, got %s", expected, string(capturedPayload))
	}
}

// TestHandle_NotPresenter は viewer ロールがハンズオン指示を拒否されることを検証する。
