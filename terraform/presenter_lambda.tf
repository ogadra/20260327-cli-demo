...
resource "aws_lambda_function" "presenter_ws" {
  ...
  environment {
    variables = {
      ...
      WEBSOCKET_API_ENDPOINT = "${aws_apigatewayv2_api.presenter_websocket.id}.execute-api.${data.aws_region}.amazonaws.com/${aws_apigatewayv2_stage.presenter_websocket.name}"
    }
  }
